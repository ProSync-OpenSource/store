...
export function ProductsSlide(props: SwiperProps) {
  ...
  const slideElement: JSX.Element = props.productsData && (
    <SwiperSlide style={{ width: 'auto' }}>
      <ProductsSlideElement>
        ...
        <div className="w-full h-full flex flex-col items-start justify-start pb-3 gap-y-2">
          <span className="text-gray-800 font-medium text-[16px] max-[900px]:text-[15px]">
            {props.productsData.product_title}
          </span>
          ...
          <div className="w-full flex flex-col items-start justify-center">
            <div className="w-full flex flex-row items-center gap-x-3">
              ...
              <span className="text-gray-800 font-medium text-sm max-[900px]:text-[13px]">
                {props.productsData.vote_average}
              </span>
            </div>

            {props.productsData.offer ? (
              <>
                <span className="text-gray-500 text-sm font-medium mt-1 leading-tight line-through max-[900px]:text-[13px]">
                  R${props.productsData.product_price}
                </span>
                <span className="text-gray-800 font-bold text-[27px] leading-tight">
                  R$
                  {`${(props.productsData.product_price * (1 - props.productsData.offer)).toFixed(2)}`.replace(
                    '.',
                    ','
                  )}
                </span>
  ...
  return (
    <SwiperContainer>
      ...
      <div className="w-full max-[800px]:hidden">
        <button
          className={`${props.navigation?.prevEl} absolute z-10 left-0 top-2/4 -translate-y-2/4 outline-none -translate-x-2/4 rounded-full w-14 h-14 flex justify-center items-center bg-base-color max-[900px]:w-12 max-[900px]:h-12`}
        >
          <Image
            src={Arrow_left}
        ...
        <button
          className={`${props.navigation?.nextEl} absolute top-2/4 z-20 -translate-y-2/4 outline-none right-0 translate-x-2/4 rounded-full bg-base-color w-14 h-14 flex justify-center items-center max-[900px]:w-12 max-[900px]:h-12`}
        >
          <Image
            src={Arrow_right}
            alt={'arrow left picture'}
